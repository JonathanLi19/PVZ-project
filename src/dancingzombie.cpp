...
dancingzombie::dancingzombie(int path_num1)
{
    ...
    setMovie("D:/QtProjects/images/Zombies/DancingZombie/SlidingStep.gif");
}
...
void dancingzombie::advance(int phase)
{
    ...
    if (hp <= 0)
    {
        ...
        {
            state = 0;
            setMovie("D:/QtProjects/images/Zombies/DancingZombie/Die.gif");
        }
...
if(x()<0)
    ...
    foreach(QGraphicsItem * item, list)//给所有范围之内的队友加生命,不攻击植物
    {
        if(item->type() == Zombie::Type)//如果是僵尸
        {
            ...
            if(zombie->hp+hurt <= zombie->total_hp)
                zombie->hp += hurt;
